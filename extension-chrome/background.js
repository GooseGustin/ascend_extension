(() => {
  ...
  var IndexedDb = class extends import_wrapper_default {
    // Tables
    users;
    quests;
    sessions;
    ...
    async getActiveQuests(userId) {
      ...
      const quests = await this.quests.where("ownerId").equals(userId).toArray();
    ...
    async getSessionsByDateRange(userId, startDate, endDate) {
      ...
      const sessions = await this.sessions.where("userId").equals(userId).toArray();
    ...
    async getTaskOrder(userId) {
      ...
      const taskOrders = await this.taskOrders.where("userId").equals(userId).first();
    ...
    async getTaskOrderForToday(userId) {
      const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      const found = await this.taskOrders.where("[userId+date]").equals([userId, today]).filter((d) => !d.questId).first();
      return found;
    }
    /**
     * Upsert task order
    ...
    async upsertTaskOrder(taskOrder) {
      const doc = {
        ...taskOrder,
        lastUpdated: taskOrder.lastUpdated ?? (/* @__PURE__ */ new Date()).toISOString()
      };
    ...
    async getPendingSyncOps(limit = 50) {
      return await this.syncQueue.orderBy("[priority+timestamp]").reverse().limit(limit).toArray();
    }
    /**
    ...
    async getHistoricalMetric(userId, metricType, periodsAgo) {
      ...
      if (metricType === "weeklyVelocity") {
        lookbackMs = 7 * 24 * 60 * 60 * 1e3;
      } else if (metricType === "monthlyConsistency") {
        lookbackMs = 30 * 24 * 60 * 60 * 1e3;
      } else {
        return void 0;
      }
      ...
      const historicalSnapshot = await this.performanceSnapshots.where("[userId+metricType+timestamp]").between([userId, metricType, targetStart], [userId, metricType, targetEnd], true, false).reverse().first();
  ...
  // src/worker/services/seed.service.ts
  var QUEST_COLORS = [
    "#4F8EF7",
    "#5CC8FF",
    "#3FA9D9",
  ...
  var SeedService = class {
    ...
    async createTestUser() {
      const user = {
        ...
        isPublic: true,
        joinDate: new Date(Date.now() - 30 * 864e5).toISOString(),
        // 30 days ago
        ...
        inventory: [
          {
            ...
            slotType: "Accessory",
            isEquipped: true,
            acquiredAt: new Date(Date.now() - 15 * 864e5).toISOString()
          },
          {
            itemId: "rune-001",
            name: "Rune of Temporal Acceleration",
            type: "Rune",
            effect: "\xD72 XP multiplier",
            rarity: "Epic",
            usesRemaining: 3,
            durability: null,
            slotType: null,
            isEquipped: false,
            acquiredAt: new Date(Date.now() - 7 * 864e5).toISOString()
          }
        ],
        equippedArtifacts: {
          weapon: null,
        ...
        streakData: {
          ...
          streakStartDate: new Date(Date.now() - 7 * 864e5).toISOString().split("T")[0]
        },
        consistencyScore: 78,
      ...
      const quests = [
        // Quest 1: Active daily quest
        {
          ...
          subtasks: [
            {
              ...
              estimatePomodoros: 3,
              isComplete: true,
              completedAt: new Date(Date.now() - 2 * 864e5).toISOString(),
              revisionCount: 0
            },
            {
              id: "subtask-002",
          ...
          progressHistory: [
            ...
            {
              date: new Date(Date.now() - 1 * 864e5).toISOString().split("T")[0],
              completions: 1,
              expEarned: 200,
              timeSpentMin: 25,
              isMilestone: false,
          ...
          tracking: {
            totalTrackedTime: 300,
            // 5 hours
            velocity: 240,
            // 1200 XP / 5 hours
            averageSessionQuality: 82,
            lastSessionAt: new Date(Date.now() - 864e5).toISOString()
          },
          registeredAt: new Date(Date.now() - 10 * 864e5).toISOString(),
        ...
        // Quest 2: Weekly project quest
        {
          ...
          progressHistory: [
            {
              date: new Date(Date.now() - 3 * 864e5).toISOString().split("T")[0],
              completions: 1,
              expEarned: 300,
              timeSpentMin: 50,
              isMilestone: false,
          ...
          tracking: {
            ...
            lastSessionAt: new Date(Date.now() - 3 * 864e5).toISOString()
          },
        ...
        // Quest 3: Todo Quest
        {
          ...
          difficulty: {
            ...
            gmValidated: "Trivial",
            isLocked: true,
            validatedAt: new Date(Date.now() - 1 * 864e5).toISOString(),
            xpPerPomodoro: 50
          },
          schedule: {
            frequency: "Daily",
          ...
          registeredAt: new Date(Date.now() - 20 * 864e5).toISOString(),
        ...
        // Quest 4: Overdue quest for testing
        {
          ...
          difficulty: {
            ...
            gmValidated: "Medium",
            isLocked: true,
            validatedAt: new Date(Date.now() - 8 * 864e5).toISOString(),
            xpPerPomodoro: 200
          },
          schedule: {
            frequency: "Weekly",
          ...
          registeredAt: new Date(Date.now() - 15 * 864e5).toISOString(),
     ...
     */
    async createHistoricalSessions(userId, quests) {
      ...
      for (const { daysAgo, count, questId, xp } of sessionsToCreate) {
        for (let i = 0; i < count; i++) {
          const sessionDate = new Date(Date.now() - daysAgo * 864e5);
          const session = {
            sessionId: `session-${daysAgo}-${i}-${Date.now()}`,
            userId,
            questId,
            subtaskId: null,
            startTime: new Date(sessionDate.getTime() + i * 36e5).toISOString(),
            // Spread throughout day
            endTime: new Date(sessionDate.getTime() + i * 36e5 + 15e5).toISOString(),
            // 25 min later
  ...
  // src/worker/services/gm/gm.service.ts
  var GMService = class {
    db;
    remoteApi;
    analyticsService;
    // ADDED
    questService;
    // ADDED (Assumes injection)
    static QUEUE_PROCESSING_INTERVAL = 6e4;
    // 60 seconds
    constructor(analyticsService, questService2) {
      this.db = getDB();
      this.remoteApi = new RemoteAPI();
      this.analyticsService = analyticsService;
      this.questService = questService2;
    }
    /**
    ...
    async getAgentState(userId) {
      const metrics = await this.analyticsService.generateAgentState(userId);
     ...
     */
    async queueValidation(userId, questId) {
     ...
     */
    async validateQuestRemote(userId, quest) {
      ...
      const validationResult = await this.remoteApi.validateQuest(quest, userContext);
      console.log("[GM Service] Remote validation result:", validationResult);
      if (validationResult.status === "validated" && validationResult.suggestedDifficulty) {
        quest.difficulty.gmValidated = validationResult.suggestedDifficulty;
        quest.difficulty.isLocked = true;
        quest.difficulty.validatedAt = (/* @__PURE__ */ new Date()).toISOString();
        ...
        quest.gmFeedback = {
          ...
          confidence: validationResult.confidence || 1,
     ...
     */
    async runValidationPipelineLocal(userId, questId) {
      const quest = await this.questService.getQuest(questId);
      if (!quest) throw new Error("Quest not found for local validation.");
      try {
        const agentState = await this.analyticsService.generateAgentState(userId);
        ...
        const gmFeedback = {
          ...
          // Lower confidence for local reasoning
    ...
    async processPendingQueue() {
      ...
      try {
        ...
        const pendingOps = await this.db.getPendingSyncOps(10);
     ...
     */
    mockGMValidation(context) {
      const { userAssignedDifficulty, subtaskCount, estimatedHours, agentState } = context;
      let validatedDifficulty = userAssignedDifficulty;
      let confidence = 0.5;
      let suggestedXPPerPomodoro = 50;
      let reasoning = `Initial assessment of quest "${context.questTitle}" assigned as ${userAssignedDifficulty}. `;
      let recommendations = [];
      if (agentState.weeklyVelocity < 30 && agentState.averageSessionQuality > 85) {
        if (validatedDifficulty !== "Trivial") {
          validatedDifficulty = "Trivial";
          confidence += 0.3;
          reasoning += "User profile suggests consistent overestimation of task difficulty. ";
          recommendations.push("Increase quest difficulty for higher XP gain.");
        }
      }
      if (subtaskCount > 25 && estimatedHours > 10 && userAssignedDifficulty !== "Epic") {
        if (agentState.monthlyConsistency < 60) {
          validatedDifficulty = "Hard";
          suggestedXPPerPomodoro = 75;
          confidence += 0.2;
          reasoning += "Quest size is excessive (25+ subtasks) and user consistency is low. Difficulty adjusted to Hard to reflect scope. ";
          recommendations.push(
            "Break this quest into smaller, manageable sub-quests."
          );
        }
      }
      if (agentState.burnoutRisk === "High" || agentState.burnoutRisk === "Critical") {
        if (validatedDifficulty !== "Trivial") {
      ...
      return {
        ...
        subtaskComplexity: subtaskCount > 20 ? "High" : subtaskCount > 5 ? "Medium" : "Low"
      };
    }
    /**
    ...
    async getHomeSuggestions(userId) {
      ...
      for (const quest of quests) {
        if (quest.validationStatus === "validated" && quest.gmFeedback && (quest.gmFeedback.validatedAt || "") > recentValidationThreshold) {
          const summaryText = `GM updated '${quest.title}' to ${quest.difficulty.gmValidated}. Confidence: ${(quest.difficulty.confidence * 100).toFixed(0)}%.`;
          suggestions.push({
      ...
      if (agentState?.metrics) {
        ...
        if (metrics.burnoutRisk === "High" || metrics.burnoutRisk === "Critical") {
          suggestions.push({
        ...
        if (metrics.consistencyTrend === "declining" || metrics.velocityTrend === "declining") {
          suggestions.push({
        ...
        if (metrics.weeklyVelocity > 40 && metrics.consistencyTrend === "improving") {
          suggestions.push({
  ...
  var AnalyticsService = class {
     ...
     */
    async getQuestSessions(questId, limit = 50) {
      return await this.db.sessions.where("questId").equals(questId).reverse().limit(limit).toArray();
    }
    /**
     * Calculate velocity (XP per hour) for a quest
     */
    async calculateQuestVelocity(questId) {
     ...
     */
    async getAverageQuality(questId) {
     ...
     */
    async getConsistencyScore(userId, days = 14) {
      ...
      const activeDays = new Set(
        sessions.filter((s) => s.status === "completed" && s.actualDurationMin >= 10).map((s) => s.startTime.split("T")[0])
      );
     ...
     */
    async getSessionQualityBreakdown(userId, days = 30) {
      const endDate = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      const startDate = new Date(Date.now() - days * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
      const sessions = await this.db.getSessionsByDateRange(
        userId,
     ...
     */
    async getWorkDistribution(userId, period) {
      ...
      const startDate = new Date(now.getTime() - days * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
      ...
      const quests = await this.db.quests.where("ownerId").equals(userId).toArray();
      ...
      sessions.forEach((session) => {
        ...
        if (groupBy === "day") {
          ...
          const weekNum = Math.floor(
            (date.getTime() - new Date(startDate).getTime()) / (7 * 24 * 60 * 60 * 1e3)
          ) + 1;
          key = `Week ${weekNum}`;
        }
        if (!grouped.has(key)) {
          grouped.set(key, /* @__PURE__ */ new Map());
        }
        const questTitle = questMap.get(session.questId) || "Unknown";
        const currentTime = grouped.get(key).get(questTitle) || 0;
        grouped.get(key).set(questTitle, currentTime + session.actualDurationMin);
      });
     ...
     */
    async getTimeDistribution(userId, days = 7) {
      const endDate = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      const startDate = new Date(Date.now() - days * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
      const sessions = await this.db.getSessionsByDateRange(
        userId,
      ...
      const timeByQuest = /* @__PURE__ */ new Map();
      sessions.forEach((session) => {
     ...
     */
    async getVelocityData(userId, days = 7) {
      const now = /* @__PURE__ */ new Date();
      const sessions = await this.db.getSessionsByDateRange(
        userId,
        new Date(now.getTime() - days * 24 * 60 * 60 * 1e3).toISOString().split("T")[0],
        now.toISOString().split("T")[0]
      );
      const quests = await this.db.quests.where("ownerId").equals(userId).toArray();
      const questMap = new Map(quests.map((q) => [q.questId, q.title]));
      const velocityByDay = /* @__PURE__ */ new Map();
      sessions.forEach((session) => {
        const date = new Date(session.startTime);
        const dayKey = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][date.getDay()];
        if (!velocityByDay.has(dayKey)) {
          velocityByDay.set(dayKey, /* @__PURE__ */ new Map());
        }
        const questTitle = questMap.get(session.questId) || "Unknown";
      ...
      return Array.from(velocityByDay.entries()).map(([day, questData]) => {
        ...
        questData.forEach((data, quest) => {
          entry[quest] = data.time > 0 ? Math.round(data.xp / data.time * 60) : 0;
        });
        return entry;
      });
    }
    ...
    async getProgressStats(userId) {
      ...
      return {
        ...
        xpThisMonth,
        xpRisePercent: xpLastMonth > 0 ? Math.round((xpThisMonth - xpLastMonth) / xpLastMonth * 100) : 0,
        levelRise: 0,
        // Calculate based on level tracking
     ...
     */
    async getBestQuests(userId, limit = 3) {
      const quests = await this.db.quests.where("ownerId").equals(userId).toArray();
      ...
      const questStats = await Promise.all(
        quests.map(async (quest) => {
          ...
          return {
            questId: quest.questId,
            title: quest.title,
            icon: quest.type === "DungeonQuest" ? "\u2694\uFE0F" : "\u{1F4CB}",
            ...
            totalXP,
            avgSessionTime: completedSessions.length > 0 ? Math.round(totalTime / completedSessions.length) : 0,
            consistency: 85
            // Calculate from session regularity
          };
        })
      );
     ...
     */
    async getWeakQuests(userId, limit = 3) {
      const quests = await this.db.quests.where("ownerId").equals(userId).toArray();
      const avgVelocity = 55;
      const questStats = await Promise.all(
        quests.map(async (quest) => {
          ...
          const interruptionRate = sessions.length > 0 ? Math.round(interruptedCount / sessions.length * 100) : 0;
          const issues = [];
      ...
      return questStats.filter((q) => q.issues.length > 0).sort((a, b) => a.velocity - b.velocity).slice(0, limit);
    }
    /**
    ...
    async getSessionHistory(options) {
      ...
      return await this.db.sessions.where("[userId+startTime]").between([userId, ""], [userId, "\uFFFF"]).reverse().limit(limit).toArray();
    }
    /**
     * Get sessions within a date range
     */
    async getSessionsByDateRange(userId, startDate, endDate) {
      return await this.db.sessions.where("userId").equals(userId).filter((session) => {
     ...
     */
    async calculateWeeklyVelocity(userId) {
      ...
      const totalXP = sessions.filter((s) => s.status === "completed").reduce((sum, s) => sum + (s.xpEarned || 0), 0);
      const totalHours = sessions.reduce((sum, s) => sum + s.actualDurationMin, 0) / 60;
    ...
    async calculateConsistencyScore(userId) {
      const days = 30;
      const thirtyDaysAgo = new Date(Date.now() - days * 24 * 60 * 60 * 1e3);
      const endDate = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
     ...
     */
    async assessBurnoutRisk(userId) {
      ...
      const sessionsWithQuality = sessions.filter((s) => s.quality?.score !== void 0);
      const avgSessionQuality = sessionsWithQuality.length > 0 ? sessionsWithQuality.reduce((sum, s) => sum + (s.quality.score || 0), 0) / sessionsWithQuality.length : 0;
      if (avgSessionQuality < 50 && sessionsThisWeek > 5) riskScore += 25;
      const recentSessions = sessionsWithQuality.slice(-10);
      const recentQuality = recentSessions.length > 0 ? recentSessions.reduce((sum, s) => sum + (s.quality.score || 0), 0) / recentSessions.length : avgSessionQuality;
      if (recentQuality < avgSessionQuality - 15) riskScore += 25;
      const quests = await this.db.getActiveQuests(userId);
      const overdueCount = quests.filter(
        (q) => q.dueDate && new Date(q.dueDate) < /* @__PURE__ */ new Date()
      ).length;
     ...
     */
    async generateAgentState(userId) {
      ...
      const avgQuality = sessionsToday.length > 0 ? sessionsToday.reduce((sum, s) => sum + (s.quality?.score || 0), 0) / sessionsToday.length : 0;
      const overdueCount = quests.filter(
        (q) => q.dueDate && new Date(q.dueDate) < /* @__PURE__ */ new Date()
      ).length;
      const lastWeekVelocity = await this.db.getHistoricalMetric(userId, "weeklyVelocity", 1) || 0;
      const velocityTrend = velocity > lastWeekVelocity * 1.1 ? "improving" : velocity < lastWeekVelocity * 0.9 ? "declining" : "stable";
      const lastMonthConsistency = await this.db.getHistoricalMetric(userId, "monthlyConsistency", 1) || 0;
      const consistencyTrend = consistency > lastMonthConsistency * 1.05 ? "improving" : consistency < lastMonthConsistency * 0.95 ? "declining" : "stable";
      const finalMetrics = {
  ...
  // src/worker/services/quest.service.ts
  var QuestService = class {
    ...
    constructor() {
      ...
      this.gmService = new GMService(analyticsService, this);
    }
    /**
    ...
    async saveQuest(quest) {
      ...
      await this.db.queueSync({
        ...
        data: quest,
        // Sending the full object for consistency
        priority: 7,
    ...
    async updateQuest(questId, updates) {
      ...
      const isDifficultyBeingModified = updates.difficulty && updates.difficulty.userAssigned !== quest.difficulty.userAssigned;
      ...
      const triggersRevalidation = isDifficultyBeingModified || updates.subtasks && updates.subtasks.length !== quest.subtasks.length || updates.timeEstimateHours !== void 0 && updates.timeEstimateHours !== quest.timeEstimateHours;
      const updatedQuest = {
        ...quest,
        ...updates,
        updatedAt: (/* @__PURE__ */ new Date()).toISOString()
    ...
    async getQuestComments(questId) {
      ...
      const comments = await this.db.comments.where("questId").equals(questId).sortBy("timestamp");
    ...
    async getPublicQuests(limit = 20) {
      return await this.db.quests.where("isPublic").equals(1).limit(limit).toArray();
    }
    /**
    ...
    async createQuest(questData) {
      ...
      const subtasks = questData.subtasks.filter((st) => st.title.trim()).map((st, index) => ({
      ...
      const calculatedHours = totalMinutes > 0 ? totalMinutes / 60 : 1;
      const timeEstimateHours = questData.timeEstimateHours ?? calculatedHours;
      ...
      const quest = {
        ...
        isTrackAligned: false,
        // Will be calculated based on specialization
    ...
    async addSubtask(questId, subtaskData) {
      ...
      quest.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
      await this.db.quests.put(quest);
      await this.db.queueSync({
        operation: "update",
        collection: "quests",
    ...
    async deleteQuest(userId, questId) {
      ...
      const allOrders = await this.db.taskOrders.where("questId").equals(questId).toArray();
  ...
  // src/worker/services/sync.service.ts
  ...
  // src/worker/background-window.ts
  ...
  chrome.action.onClicked.addListener(async () => {
    console.log("Ascend extension icon clicked");
