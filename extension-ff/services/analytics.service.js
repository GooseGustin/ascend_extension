...
export class AnalyticsService {
    ...
    async getConsistencyScore(userId, days = 14) {
        ...
        const score = Math.min(100, Math.round(baseScore + streakBonus + frequencyBonus));
        // Determine status
        let status;
        if (score >= 75)
            status = "Rising";
    ...
    async getWorkDistribution(userId, period) {
        ...
        sessions.forEach((session) => {
            ...
            if (groupBy === "day") {
                ...
                // Week number
                const weekNum = Math.floor((date.getTime() - new Date(startDate).getTime()) /
                    (7 * 24 * 60 * 60 * 1000)) + 1;
                key = `Week ${weekNum}`;
            }
            if (!grouped.has(key)) {
    ...
    async getTimeDistribution(userId, days = 7) {
        ...
        const quests = await this.db.quests
            ...
            .equals(userId)
            .toArray();
        const questMap = new Map(quests.map((q) => [q.questId, { title: q.title, color: q.color }]));
        // Sum time per quest
        const timeByQuest = new Map();
        sessions.forEach((session) => {
    ...
    async getVelocityData(userId, days = 7) {
        const now = new Date();
        const sessions = await this.db.getSessionsByDateRange(userId, new Date(now.getTime() - days * 24 * 60 * 60 * 1000)
            .toISOString()
            .split("T")[0], now.toISOString().split("T")[0]);
        const quests = await this.db.quests
            .where("ownerId")
            .equals(userId)
            .toArray();
    ...
    async getProgressStats(userId) {
        ...
        const lastMonthEnd = new Date(now.getFullYear(), now.getMonth(), 0)
            .toISOString()
            .split("T")[0];
        const thisMonthSessions = await this.db.getSessionsByDateRange(userId, thisMonthStart, now.toISOString().split("T")[0]);
        const lastMonthSessions = await this.db.getSessionsByDateRange(userId, lastMonthStart, lastMonthEnd);
        const xpThisMonth = thisMonthSessions.reduce((sum, s) => sum + s.xpEarned, 0);
        const xpLastMonth = lastMonthSessions.reduce((sum, s) => sum + s.xpEarned, 0);
        return {
            currentLevel: userProfile.totalLevel,
            totalXP: userProfile.experiencePoints,
            xpThisMonth,
            xpRisePercent: xpLastMonth > 0
                ? Math.round(((xpThisMonth - xpLastMonth) / xpLastMonth) * 100)
                : 0,
            levelRise: 0, // Calculate based on level tracking
            sessionsThisMonth: thisMonthSessions.filter((s) => s.status === "completed").length,
    ...
    async getBestQuests(userId, limit = 3) {
        ...
        const questStats = await Promise.all(quests.map(async (quest) => {
            const sessions = await this.getQuestSessions(quest.questId, 100);
            const completedSessions = sessions.filter((s) => s.status === "completed");
            const totalTime = sessions.reduce((sum, s) => sum + s.actualDurationMin, 0);
            ...
            return {
                ...
                completionRate: sessions.length > 0
                    ...
                    : 0,
                totalXP,
                avgSessionTime: completedSessions.length > 0
    ...
    async getWeakQuests(userId, limit = 3) {
        ...
        const questStats = await Promise.all(quests.map(async (quest) => {
            ...
            const totalXP = sessions.reduce((sum, s) => sum + s.xpEarned, 0);
            const velocity = totalTime > 0 ? (totalXP / totalTime) * 60 : 0;
            const interruptedCount = sessions.filter((s) => s.interruptions.length > 0).length;
            const interruptionRate = sessions.length > 0
                ? Math.round((interruptedCount / sessions.length) * 100)
                : 0;
            const issues = [];
            if (velocity < avgVelocity * 0.8)
                issues.push("Below average velocity");
            if (interruptionRate > 30)
                issues.push("High interruptions");
            if (quest.subtasks.length > 20)
                issues.push("Too many subtasks");
    ...
    async assessBurnoutRisk(userId) {
        ...
        const avgSessionQuality = sessionsWithQuality.length > 0
            ...
            : 0;
        if (avgSessionQuality < 50 && sessionsThisWeek > 5)
            riskScore += 25; // Poor overall quality
        ...
        const recentQuality = recentSessions.length > 0
            ...
            : avgSessionQuality;
        if (recentQuality < avgSessionQuality - 15)
            riskScore += 25; // Significant declining quality trend
        ...
        const overdueCount = quests.filter(q => q.dueDate && new Date(q.dueDate) < new Date()).length;
        if (overdueCount > 3)
            riskScore += 20;
    ...
    async generateAgentState(userId) {
        ...
        const profile = await this.db.users.get(userId);
        // Use Quest[] type
        const quests = await this.db.getActiveQuests(userId); // Active quests
