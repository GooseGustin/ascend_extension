import { getDB } from "../../db/indexed-db"; // ADDED IndexedDb
import { RemoteAPI, } from "../../api/remote-client"; // Existing
/**
...
export class GMService {
    constructor(analyticsService, questService) {
        // ADDED dependencies
        this.db = getDB();
    ...
    async runValidationPipelineLocal(userId, questId) {
        const quest = await this.questService.getQuest(questId);
        if (!quest)
            throw new Error("Quest not found for local validation.");
        try {
            // 1. Gather Context
    ...
    async processPendingQueue() {
        ...
        const gmOps = pendingOps.filter((op) => op.collection === "gm_validation" && op.operation === "validate");
        if (gmOps.length === 0)
            return;
        console.log(`GMService: Processing ${gmOps.length} pending validation requests.`);
        for (const op of gmOps) {
            ...
            try {
                ...
                console.warn(`GM Remote Validation failed for ${op.documentId}. Falling back to local reasoning.`);
                try {
                    ...
                    console.error(`GM Local Validation also failed for ${op.documentId}.`, localError);
                }
            }
        }
    }
    ...
    mockGMValidation(context) {
        ...
        if (agentState.weeklyVelocity < 30 &&
        ...
        if (subtaskCount > 25 &&
            ...
            if (agentState.monthlyConsistency < 60) {
                ...
                recommendations.push("Break this quest into smaller, manageable sub-quests.");
            }
        }
        // Rule 3: Burnout risk adjustment
        if (agentState.burnoutRisk === "High" ||
            ...
            if (validatedDifficulty !== "Trivial") {
                ...
                recommendations.push("Consider taking a short break or reducing your active quest count.");
            }
        }
    ...
    async getHomeSuggestions(userId) {
        ...
        const recentValidationThreshold = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
        // 1. Extract Recommendations from recently validated quests
        for (const quest of quests) {
            if (quest.validationStatus === "validated" &&
                quest.gmFeedback &&
        ...
        if (agentState?.metrics) {
            ...
            if (metrics.consistencyTrend === "declining" ||
...
GMService.QUEUE_PROCESSING_INTERVAL = 60000; // 60 seconds
